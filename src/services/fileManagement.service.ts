...
export class FileManagementService {
  ...
  moveFileFromTempToUpload(type:MediaType,filename:string,ownerIdentities:string){
    console.log("Enter")
    const fileStation=join(process.cwd(),"public",'temp',`${filename}`)
    const destinationFolder=`uploads/${ownerIdentities}/${type.toLowerCase()}`
    const destinationPath=join(process.cwd(),"public",destinationFolder,filename)
